...
<div class="section">
    ...
    <?php if ($editingProgram !== null): ?>
        ...
        if ($programInfo !== null):
            // ====== EXTRAER HORARIOS DESDE AZURACAST ======
            // Para programas no-live, obtener horarios desde AzuraCast
            $currentType = $programInfo['playlist_type'] ?? 'program';
            $isLiveProgram = ($currentType === 'live');

            // Si NO es programa live y NO tiene schedule_slots, intentar obtener desde AzuraCast
            if (!$isLiveProgram && empty($programInfo['schedule_slots']) && empty($programInfo['schedule_days'])) {
                $schedule = getAzuracastSchedule($username, 600); // 10 minutos de caché

                if ($schedule !== false && !empty($schedule)) {
                    // Buscar nombre original del programa (sin sufijos)
                    $programName = getProgramNameFromKey($editingProgram);

                    // Extraer eventos que coincidan con este programa
                    $programEvents = [];
                    foreach ($schedule as $event) {
                        $eventName = $event['name'] ?? $event['playlist'] ?? '';
                        if ($eventName === $programName) {
                            ...
                            if ($start !== null) {
                                ...
                                $programEvents[] = [
                                    'day' => (int)$startDateTime->format('w'),
                    ...
                    // Agrupar eventos por día+hora+duración para eliminar duplicados
                    $uniqueSlots = [];
                    foreach ($programEvents as $evt) {
                        $key = $evt['time'] . '_' . $evt['duration'];
                        ...
                        if (!in_array($evt['day'], $uniqueSlots[$key]['days'])) {
                            $uniqueSlots[$key]['days'][] = $evt['day'];
                        }
                    }

                    // Convertir a schedule_slots
                    if (!empty($uniqueSlots)) {
                        $programInfo['schedule_slots'] = array_values($uniqueSlots);
                    }
                }
            }
        ?>
