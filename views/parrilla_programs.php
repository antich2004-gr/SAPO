...
if (!function_exists('getAllProgramsWithStats')) {
    echo '<p style="color: red;">ERROR FATAL: La función getAllProgramsWithStats() no está definida!</p>';
    echo '<p style="color: orange;">Funciones disponibles: ' . implode(', ', get_defined_functions()['user']) . '</p>';
    $programsData = ['programs' => [], 'last_sync' => null, 'total' => 0, 'complete' => 0, 'partial' => 0, 'empty' => 0];
} else {
    echo '<p style="color: green; font-size: 11px;">DEBUG: Función getAllProgramsWithStats existe</p>';
    try {
